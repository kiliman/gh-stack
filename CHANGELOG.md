...
### 🐛 Fixes
- **`merge … --dry-run` now reports a branch missing its PR number instead of previewing a plan that would fail.** The dry-run exited on the `[DRY RUN]` line _before_ the missing-PR validation, so a preview looked clean while the real merge later stopped on the same stack. The check now runs ahead of the dry-run exit, so previews surface the same blocker as a real merge. (#27 — gh-stack's first outside contribution, thanks to Benjamin Hargett.)
